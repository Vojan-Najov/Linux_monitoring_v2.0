...
echo -n "Enter the directory where the logs are located (or 'q' for quite): "
read directory
if [ "X$directory" = "Xq" ]; then
  exit 0
fi
if [ ! -d "$directory" ]; then
  echo "Error: incorrect directory '$directory'."
fi
goaccess -f "$directory"/*.log -a -d -o /home/ccartman/nginx/html/index.html --real-time-html  --log-format=COMBINED
